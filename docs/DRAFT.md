...
| 8 | TIER-1 | OWN write плато + регрессия −18 % на 16 t | `36.2 → 29.8 M`; 16 сегментных локов |
...
локов (§4); решение — повысить `segmentCount`. Скачок `2t > 1t` у SYNC —
biased-lock / contention warmup, типичный для `synchronized`.
...
- **Write-heavy → только JDK.** OWN упирается в 16 сегментных локов (плато
  36 M, ×2.3), JDK даёт 111 M (×7.4). Если OWN нужен под запись —
  **поднять `segmentCount`** (хотя бы до `4 × ядра`).
...
единственное узкое место — фиксированные 16 сегментов на записи, что
прямо видно на `WriteBenchmark` и в flame-графах.
...
- Параметризовать `segmentCount` в `WriteBenchmark` — показать, что
  плато сдвигается вправо с ростом числа сегментов.
