...
падает до ≈ 37 % (изоляция вклада — `DRAFT.md` §15). Полные таблицы §3–9
сняты **на этой реализации** (resize-ON, JDK 21).
...
`-Xmx24g -Xms256m -XX:+UseG1GC`, OpenJDK 21.0.11.
...
sample** поверх 1 M записей. Медианная латентность ≈ 50–60 нс (не 20 нс):
случайный ключ обычно промахивается мимо L1, в эти 50–60 нс уже входит
...
**Tier 1→2 (структурные, теперь мягкие):**
- OWN записи скейлятся ×4.55 до 8 потоков благодаря динамическому
  fair-resize числа сегментов (§2.1): на 1 M записей раскладка растёт до
  64 сегментов, потолок 16 локов снят. Осталась мягкая −11 % просадка на
  16 потоках (SMT-давление на 64 стрипах). Изоляция вклада — `DRAFT.md`
  §15 (+67 % OFF→ON).
- OWN mixed-регрессия −12 % на 16 потоках при write-доминирующих смесях
  (rs0.2/0.5); при rs=0.8 её нет — те же write-стрипы под SMT.

**Tier 2 (методологические артефакты):**
- `readOwnSingle_thr01` (53 M) < `read_thr01` OWN (60 M) — dual-фикстура
...
- Scaling-фикстура (2 с итерации) слегка занижает относительно
  `ReadBenchmark` (10 с) — OWN сходится в ±0 %, JDK −9 %; SYNC сильнее
  (−41 %) под глобальным локом.

**Tier 3 (шум измерения, `fork = 2`):**
- `read_thr01/02` JDK имеют 25 %/41 % CI (бимодальный JIT-инлайнинг между
  форками) — поэтому «суперлинейный» ×9 на 8 потоках для JDK — артефакт
