...
- **Реализация:** [`hashmap.ConcurrentHashMap`](../src/main/kotlin/hashmap/ConcurrentHashMap.kt) —
  ...
  чтения (`AtomicReferenceArray` + `@Volatile`).
...
OWN масштабируется только в ×2.3 до 8 потоков и проседает на 18 % на 16:
16 сегментных `ReentrantLock`-ов, вероятность коллизии по парадоксу дней
рождений ≈ 88 % при 8 потоках. JDK CAS-ит на уровне бакетов (≈ 2 M бинов
на 1 M записей), коллизии редки → ×7.4. Flame-графы подтверждают:
...
- **Write-heavy:** только JDK; OWN ограничен 16 сегментами — поднять
  `segmentCount` под write-нагрузку.
- **SYNC** — не использовать под конкуренцией.
- **Корректность** OWN подтверждена `jcstress` + JUnit.
